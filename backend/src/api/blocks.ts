...
class Blocks {
  ...
  private async $getBlockExtended(block: IEsploraApi.Block, transactions: TransactionExtended[]): Promise<BlockExtended> {
    ...
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'drip'].includes(config.MEMPOOL.NETWORK)) {
      let pool: PoolTag;
  ...
  public async $getBlock(hash: string, skipMemoryCache: boolean = false): Promise<BlockExtended | IEsploraApi.Block> {
    ...
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'drip'].includes(config.MEMPOOL.NETWORK) === false) {
  ...
  public async $getBlockAuditSummary(hash: string): Promise<BlockAudit | null> {
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'drip'].includes(config.MEMPOOL.NETWORK) && Common.auditIndexingEnabled()) {
